...
impl AtomSet {
  pub fn intersect(lhs: &AtomSet, rhs: &AtomSet) -> AtomSet {
    match (lhs, rhs) {
      (AtomSet::LoopSet(d1), AtomSet::LoopSet(d2)) => AtomSet::LoopSet(Dag::intersect(d1, d2)),
      (AtomSet::SubStrSet(i1, j1, k1), AtomSet::SubStrSet(i2, j2, k2)) => {
        if *i1 == *i2 {
          AtomSet::SubStrSet(
            *i1,
            PositionSet::intersect(j1, j2),
            PositionSet::intersect(k1, k2),
          )
        } else {
          AtomSet::Empty
        }
...
impl PositionSet {
  pub fn intersect(lhs: &PositionSet, rhs: &PositionSet) -> PositionSet {
    match (lhs, rhs) {
      ...
      (PositionSet::Pos(p1, q1, c1), PositionSet::Pos(p2, q2, c2)) => PositionSet::Pos(
        RegExpSet::intersect(p1, p2),
        RegExpSet::intersect(q1, q2),
        IntegerExpr::intersect(c1, c2),
      ),
      _ => PositionSet::Empty,
    }
  }
  ...
  pub fn intersect(lhs: &RegExpSet, rhs: &RegExpSet) -> RegExpSet {
    if lhs.len() == rhs.len() {
      ...
      for i in 0..lhs.len() {
        let lhs = lhs.get(i);
        let rhs = rhs.get(i);
        tokens.push(Token::intersect(lhs, rhs));
      }
